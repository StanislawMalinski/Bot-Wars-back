...
public class RoleNameToCreateAdminReqirementHandler : AuthorizationHandler<RoleNameToCreateAdminRequirement, int>
{
    ...
    {
        if (requestedId == 1) return true;

        if (userRoleName is null) return false;
        return expectedRoleName.Equals(userRoleName);
    }

    protected override Task HandleRequirementAsync(AuthorizationHandlerContext context,
        RoleNameToCreateAdminRequirement requirement, int idRequestedToBeCreated)
    {
        ...
        if (roleMatch) context.Succeed(requirement);
