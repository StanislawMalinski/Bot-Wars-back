...
public class MatchMapper
{
    public MatchResponse MapEntityToResponse(Matches match)
    {
        MatchResponse matchResponse = new(match.Id, match.GameId, match.Game?
            ...
            , match.MatchPlayers.Select(x => new BotPlayer(x.BotId, x.Bot.BotFile, x.Bot.Player.Login)).ToList(),
    ...
    {
        MatchInTournamentRespond matchResponse = new(match.Id,
            match.MatchPlayers.Select(x => new BotPlayer(x.BotId, x.Bot.BotFile, x.Bot.Player.Login)).ToList()
            , match.Status.ToString(), match.Winner,
            int.Parse(match.Data), match.Played, match.MatchResult.ToString());
