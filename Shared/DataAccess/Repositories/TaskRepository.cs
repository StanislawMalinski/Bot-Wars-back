...
{
    private readonly DataContext _taskDataContext;
    ...
    public async Task<EntityEntry<_Task>> AddTask(TaskTypes type, long operatingOn, DateTime scheduledOn,
    ...
    public async Task<EntityEntry<_Task>> AddTask(TaskTypes type, long operatingOn, DateTime scheduledOn, int enginId,
        TaskStatus status = TaskStatus.Unassigned)
    {
        var task = new _Task
        {
