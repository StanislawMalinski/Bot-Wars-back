...
public class MatchRepository
{
    ...
    {
        return (await _dataContext.MatchPlayers.Where(x => x.MatchId == matchId).Include(x => x.Bot).ToListAsync())
            .ConvertAll(x => x.Bot)!;
    }
    ...
    {
        return await _dataContext.Matches.Where(x => x.TournamentsId == tourId && x.Status == GameStatus.Played)
            .Select(x => x.Id).ToListAsync();
    }
    ...
    {
        return await _dataContext.MatchPlayers.Where(x => x.MatchId == matchId && x.BotId != winner).Include(x => x.Bot)
            .Select(x => x.Bot.PlayerId).ToListAsync();
    }
    ...
    {
        return await _dataContext.MatchPlayers.Where(x => x.MatchId == matchId).Include(x => x.Bot).Select(x => x.Bot)
            .ToListAsync();
    }
    ...
    {
        return await _dataContext.Matches.Where(x => x.Id == matchId).Include(x => x.Tournament)
            .Select(x => x.Tournament).FirstAsync();
    }
