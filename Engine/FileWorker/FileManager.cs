...
public class FileManager
{
    ...
    private readonly string BotFilePath = "FileSystem/Bots";
    private readonly string GameFilePath = "FileSystem/Games";
    ...
    public async Task<HandlerResult<SuccessData<FileDto>, IErrorResult>> GetBotFileFromStorage(Bot bot)
    {
        try
        {
            ...
            var cont = (await _fileRepository.FormFileToString(res.Match(x => x.Data, null))).Match(x => x.Data, null);
            if (cont == null)
                return new EntityNotFoundErrorResult
                {
                    Title = "EntityNotFoundErrorResult",
    ...
    {
        var file = await GetFileFromStorage(fileId);
        var newGamePath = GameFilePath + "/" + id;
    ...
    private async Task<string> GetFileFromStorage(long fileId)
    {
        ...
        if (!res.IsSuccess) throw new ArgumentException("File not found in Gatherer!");
        var cont = (await _fileRepository.FormFileToString(res.Match(x => x.Data, null))).Match(x => x.Data, null);
        if (cont == null) throw new ArgumentException("File is null!");
    ...
    private void SaveFileAs(string where, string filename, string fileContent, Language language)
    {
        ...
        {
            case Language.C:
                File.WriteAllText($"{where}/{Path.GetFileNameWithoutExtension(filename)}.cpp", fileContent);
                break;
            case Language.PYTHON:
                File.WriteAllText($"{where}/{Path.GetFileNameWithoutExtension(filename)}.py", fileContent);
                break;
            case Language.Java:
                File.WriteAllText($"{where}/{Path.GetFileNameWithoutExtension(filename)}.java", fileContent);
                break;
        }
    }
            ...
            switch (bot.Language)
            {
                case Language.C:
                    dirs = Directory.GetFiles($"{BotFilePath}/{bot.Id}",
                        $"{Path.GetFileNameWithoutExtension(bot.BotFile)}.cpp");
    ...
    public async Task<string> GetGameFilepath(Game game)
    {
        ...
        if (Directory.GetDirectories(GameFilePath, game.Id.ToString()).Length != 0)
        {
            ...
            {
                case Language.C:
                    dirs = Directory.GetFiles($"{GameFilePath}/{game.Id}",
                        $"{Path.GetFileNameWithoutExtension(game.GameFile)}.out");
                    if (dirs.Length != 0) return $"{GameFilePath}/{game.Id}";

                    break;
                case Language.PYTHON:
                    dirs = Directory.GetFiles($"{GameFilePath}/{game.Id}",
                        $"{Path.GetFileNameWithoutExtension(game.GameFile)}.py");
                    if (dirs.Length != 0) return $"{GameFilePath}/{game.Id}";
                    break;
                case Language.Java:
                    dirs = Directory.GetFiles($"{GameFilePath}/{game.Id}",
                        $"{Path.GetFileNameWithoutExtension(game.GameFile)}.java");
                    if (dirs.Length != 0) return $"{GameFilePath}/{game.Id}";
                    break;
            }
        }
    ...
    {
        var filePath = where + "/" + id + Path.GetExtension(file.FileName);
    ...
    {
        var file = _fileRepository.StringToFormFile(log, name, "text/plain").Match(x => x.Data, null);
