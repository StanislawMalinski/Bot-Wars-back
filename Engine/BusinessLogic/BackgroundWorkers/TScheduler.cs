...
public class TScheduler : IInvocable
{
    ...
    public TScheduler(SchedulerRepository schedulerRepository, TaskRepository taskRepository, IScheduler scheduler,
            ...
            switch (t.Type)
            {
                ...
                case TaskTypes.PlayGame:
                    if (await _schedulerRepository.Taskdoing(t.Id))
                    {
                        ...
                        _scheduler.ScheduleWithParams<GameWorker>(t.Id).EverySecond().Once()
                            .PreventOverlapping("game worker " + t.Id);
                    }
